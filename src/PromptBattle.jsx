  ...
  intermediate: [
    {
      ...
      scenario: "Get Claude to act as a tough-but-fair venture capitalist and critique your business idea of 'an app that reminds you to drink water.'",
    ...
    },
    {
      ...
      scenario: "Get Claude to produce a weekly meal plan for a busy professional — but make it actually usable and scannable.",
    ...
    },
    {
      ...
      scenario: "Get Claude to write the same rejection email in three completely different tones: corporate, Gen Z casual, and Shakespearean.",
      hint: "Think about: how do you specify tone? What anchors do you give Claude to nail each one?",
  ...
  advanced: [
    {
      ...
      scenario: "Get Claude to analyse why remote work might actually be hurting junior employees' careers — using a structured framework it builds itself.",
      hint: "Think about: chain-of-thought prompting, asking Claude to reason before concluding, framework construction",
      evaluationFocus: "reasoning scaffolding, framework elicitation, depth of analysis"
    },
    {
      id: "a2", title: "THE CRITIC'S CRITIC",
      scenario: "Get Claude to write a social media post, then immediately critique it as a harsh editor, then rewrite it based on the critique.",
    ...
    },
    {
      ...
      scenario: "Build a prompt that turns Claude into a specific, memorable AI persona for a fictional brand — consistent across any question asked.",
...
async function judgePrompt(challenge, userPrompt, level) {
  ...
  const response = await fetch(apiUrl, {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1000,
      system: `You are an expert AI prompt evaluator for BigSpaceAI, an AI education company in Singapore. Evaluate prompts written by students learning AI prompting.
...
// Grid texture SVG as background
const GridBg = () => (
  <div style={{
...
export default function PromptBattle() {
  ...
  if (screen === "home") return wrap(
    <div style={{ animation: "fadeUp 0.6s ease forwards", paddingTop: "64px" }}>
      <div style={{
        fontFamily: "'Barlow', sans-serif", fontSize: "11px", letterSpacing: "0.25em",
        color: RED, textTransform: "uppercase", marginBottom: "20px"
      }}>
        AI CREATOR TOOLKIT
      </div>
      <h1 style={{
        fontFamily: "'Anton', sans-serif", fontSize: "clamp(56px, 13vw, 96px)",
        lineHeight: "0.95", letterSpacing: "0.02em", marginBottom: "8px", color: WHITE
      }}>
        PROMPT<br /><span style={{ color: RED }}>BATTLE.</span>
      </h1>
      <div style={{ width: "60px", height: "4px", background: RED, margin: "24px 0" }} />
      <p style={{
        fontFamily: "'Barlow', sans-serif", fontSize: "17px", lineHeight: "1.7",
        color: GREY, maxWidth: "480px", marginBottom: "48px"
      }}>
        Test your AI prompting skills. Write prompts. Get judged by Claude. Earn your badge.
      </p>
      ...
      <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "52px" }}>
        {["✍️ WRITE A PROMPT", "🤖 CLAUDE RUNS IT", "📊 GET SCORED", "🏆 EARN A BADGE"].map((s, i) => (
          <div key={i} style={{
            border: "1px solid #222", padding: "8px 14px",
            fontFamily: "'Anton', sans-serif", fontSize: "11px",
            letterSpacing: "0.1em", color: GREY
          }}>{s}</div>
        ))}
      </div>

      <button onClick={() => setScreen("level")} style={{
        background: RED, border: "none", color: WHITE,
        fontFamily: "'Anton', sans-serif", fontSize: "18px", letterSpacing: "0.12em",
        padding: "18px 48px", cursor: "pointer", display: "flex", alignItems: "center", gap: "12px"
      }}>
        START BATTLE →
      </button>
    ...
    <div style={{ animation: "fadeUp 0.5s ease forwards", paddingTop: "48px" }}>
      ...
      {Object.entries(LEVELS).map(([key, cfg], i) => (
        <button key={key} onClick={() => pickChallenge(key)} style={{
          ...
          padding: "24px 28px", marginBottom: "12px", cursor: "pointer",
    ...
    return wrap(
      <div style={{ animation: "fadeUp 0.5s ease forwards", paddingTop: "40px" }}>
        <div style={{ display: "flex", gap: "10px", alignItems: "center", marginBottom: "32px" }}>
          <span style={{
            background: RED, fontFamily: "'Anton', sans-serif", fontSize: "11px",
            letterSpacing: "0.15em", color: WHITE, padding: "4px 12px"
          }}>{cfg.emoji} {cfg.label}</span>
          <span style={{ fontFamily: "'Barlow', sans-serif", fontSize: "12px", color: GREY, letterSpacing: "0.08em", textTransform: "uppercase" }}>
            {challenge.title}
          </span>
        </div>

        <div style={{ border: "1px solid #222", borderTop: `3px solid ${RED}`, padding: "28px", marginBottom: "20px" }}>
          <div style={{ fontFamily: "'Anton', sans-serif", fontSize: "11px", letterSpacing: "0.2em", color: RED, marginBottom: "16px" }}>
            YOUR CHALLENGE
          </div>
          <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: "17px", lineHeight: "1.75", color: WHITE, marginBottom: "24px", fontWeight: "500" }}>
            {challenge.scenario}
          </p>
        ...
        <div style={{ border: "1px solid #222", padding: "28px", marginBottom: "20px" }}>
          ...
          <textarea
            ...
            style={{
              width: "100%", minHeight: "160px", background: "#0f0f0f",
              border: "1px solid #2a2a2a", color: WHITE, fontSize: "15px",
              lineHeight: "1.7", padding: "16px", fontFamily: "'Barlow', sans-serif",
              resize: "vertical"
            }}
          />
  ...
  if (screen === "results" && results) {
    ...
    const gradeColor = GRADE_COLORS[results.grade] || WHITE;
    ...
    return wrap(
      <div style={{ animation: "fadeUp 0.5s ease forwards", paddingTop: "40px" }}>
        ...
        <div style={{ borderTop: `4px solid ${RED}`, background: "#0d0d0d", padding: "40px", marginBottom: "16px", textAlign: "center" }}>
          <div style={{ fontFamily: "'Anton', sans-serif", fontSize: "11px", letterSpacing: "0.25em", color: GREY, marginBottom: "16px" }}>
            YOUR SCORE
          </div>
          <div style={{
            fontFamily: "'Anton', sans-serif",
            fontSize: "clamp(80px, 20vw, 120px)",
            lineHeight: "1", color: passed ? RED : WHITE,
            marginBottom: "8px"
          }}>
            {results.total}
          </div>
          <div style={{ fontFamily: "'Barlow', sans-serif", fontSize: "13px", color: GREY, marginBottom: "20px" }}>
            out of 100 · Pass mark: {cfg.passMark}
          </div>
          <span style={{
            background: passed ? RED : "#1a1a1a",
            border: `1px solid ${passed ? RED : "#333"}`,
            fontFamily: "'Anton', sans-serif", fontSize: "14px",
            letterSpacing: "0.15em", color: WHITE, padding: "8px 20px"
          }}>
        ...
        <div style={{ border: "1px solid #1f1f1f", borderLeft: `4px solid ${RED}`, padding: "28px", marginBottom: "16px" }}>
          ...
          <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: "16px", lineHeight: "1.8", color: "#ccc", marginBottom: "28px" }}>
            {results.verdict}
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "24px" }}>
            <div>
              <div style={{ fontFamily: "'Anton', sans-serif", fontSize: "11px", letterSpacing: "0.15em", color: WHITE, marginBottom: "12px" }}>
                ✓ STRENGTHS
              </div>
              {results.strengths.map((s, i) => (
                <div key={i} style={{
                  fontFamily: "'Barlow', sans-serif", fontSize: "13px", color: GREY,
                  lineHeight: "1.6", marginBottom: "8px", paddingLeft: "12px",
        ...
        {/* Badge */}
        <div style={{
          border: `2px solid ${passed ? RED : "#222"}`,
          background: passed ? "rgba(235,29,37,0.06)" : "#0d0d0d",
          ...
          {passed && (
            <div style={{ fontFamily: "'Barlow', sans-serif", fontSize: "12px", color: GREY }}>
              BigSpaceAI Certified · {new Date().toLocaleDateString("en-SG", { month: "short", year: "numeric" })}
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
          <button onClick={() => pickChallenge(level)} style={{
            background: RED, border: "none", color: WHITE,
            fontFamily: "'Anton', sans-serif", fontSize: "15px", letterSpacing: "0.12em",
            padding: "16px 32px", cursor: "pointer", flex: 1
          }}>TRY ANOTHER →</button>
          <button onClick={() => setScreen("level")} style={{
            background: "transparent", border: "1px solid #333", color: GREY,
            fontFamily: "'Anton', sans-serif", fontSize: "15px", letterSpacing: "0.12em",
